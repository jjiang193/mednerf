...
from graf.datasets import *

from graf.utils import polar_to_cartesian, look_at, to_phi, to_theta
import sys
import copy
import argparse
import os
...
from graf.utils import to_phi, to_theta, save_video, InfiniteSamplerWrapper
...
def reconstruct(args, config_file):
    ...
    print(args.xray_img_path)
    target_xray = glob.glob(os.path.join(args.xray_img_path, '*.png'))
    target_xray = torch.unsqueeze(trans(Image.open(r'C:\micro-CT\tiankuo\Emily Wang batch 2\reconstruction\week 0\926\x.png').convert('RGB')),0)
...
if __name__ == "__main__":
    ...
    eval_dir = os.path.join(out_dir, "./renderings")

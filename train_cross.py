...
if __name__ == '__main__':
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(dataset_list)):
        ...
        # Split the dataset
        train_subset = [dataset_list[i] for i in train_index]
        val_subset = [dataset_list[i] for i in val_index]
        ...
        train_loader = iter(torch.utils.data.DataLoader(
            ...
            batch_size=batch_size,
            num_workers=config['training']['nworkers'],
            shuffle=False, pin_memory=True, sampler=InfiniteSamplerWrapper(train_subset)
        ))
        ...
        # Initialize fid+kid evaluator for this fold
        if fid_every > 0:
            fid_cache_file = os.path.join(out_dir, f'fid_cache_train_fold_{fold}.npz')
            kid_cache_file = os.path.join(out_dir, f'kid_cache_train_fold_{fold}.npz')
            evaluator.inception_eval.initialize_target(val_loader, cache_file=fid_cache_file, act_cache_file=kid_cache_file)
        ...
        while True:
            ...
            for x_real in train_loader:
                ...
                # Compute FID and KID on validation set
                if fid_every > 0 and ((it + 1) % fid_every) == 0:
                    fid, kid = evaluator.compute_fid_kid()
                    print(f'Fold {fold + 1}, FID: {fid:.4f}, KID: {kid:.4f}')
                    logger.add('validation', f'fid_fold_{fold}', fid, it=it)
                    logger.add('validation', f'kid_fold_{fold}', kid, it=it)
                
                # ... (rest of the existing code)
                
                # Break condition for each fold (you may want to adjust this)
                if it >= config['training']['n_iterations_per_fold']:
                    break
            
            # Save model for each fold
            checkpoint_io.save(f'model_fold_{fold}.pt', it=it, epoch_idx=epoch_idx, fid_best=fid_best, kid_best=kid_best)
            logger.save_stats(f'stats_fold_{fold}.p')

    # After all folds, compute average metrics
    fid_scores = [logger.get_last('validation', f'fid_fold_{i}') for i in range(5)]
    kid_scores = [logger.get_last('validation', f'kid_fold_{i}') for i in range(5)]
    avg_fid = sum(fid_scores) / len(fid_scores)
    avg_kid = sum(kid_scores) / len(kid_scores)
    print(f"Average FID across folds: {avg_fid:.4f}")
    print(f"Average KID across folds: {avg_kid:.4f}")